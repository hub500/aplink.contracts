...
struct FARM_TBL leaselist_t {
    ...
    leaselist_t() {}
    leaselist_t(const uint64_t& pid): id(pid) {}

    uint64_t primary_key() const { return id; }

    uint128_t by_tenant() const { return (uint128_t)tenant.value << 64 | (uint128_t)id; }
    uint64_t by_updatedid() const { return ((uint64_t)updated_at.sec_since_epoch() << 32) | (id & 0x00000000FFFFFFFF); }
    typedef eosio::multi_index<"leaselist"_n, leaselist_t,
        indexed_by<"tenantidx"_n,  const_mem_fun<leaselist_t, uint128_t, &leaselist_t::by_tenant> >,
        indexed_by<"updatedid"_n,  const_mem_fun<leaselist_t, uint64_t, &leaselist_t::by_updatedid> >
    > idx_t;

    EOSLIB_SERIALIZE( leaselist_t,  (id)(tenant)(land_title)(land_uri)(banner_uri)(alloted_apples)(available_apples)
                                (status)(desc_cn)(desc_en)(opened_at)(closed_at)(created_at)(updated_at) )
};
